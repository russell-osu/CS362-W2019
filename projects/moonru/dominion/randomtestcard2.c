/******************************************************************************
 * Name: Russell Moon
 * Program: Random Card Test 2 (Great Hall)
 * Description: Tests the Great Hall card from dominion.c with randomized 
 * input
 * Class: CS 362
...
int main()
{
    ...
    for (int k = 0; k < numTests; k++)
    {
        ...
        cardEffect(great_hall, choice1, choice2, choice3, &G, handPos, &bonus); //play Great Hall
        ...
        //test function versus expected value
        printf("P0 Num actions: %d; Expected: %d -> ", G.numActions, ConG.numActions + 1);
        if (assertTrue(G.numActions, ConG.numActions + 1, &testsFailed))
            printf("PASSED\n");
        else
            printf("FAILED\n");
