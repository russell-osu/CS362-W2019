...
/*
*   MINION EFFECT REFACTOR
*/
void minionEffect(int currentPlayer, int choice1, int choice2, int handPos, struct gameState *state){
  int i, j;
  //+1 action
  state->numActions++;
    ...
    for (i = 0; i < state->numPlayers; i++){
      if (i != currentPlayer){
        if ( state->handCount[i] > 4 ){
          //discard hand
          while( state->handCount[i] >= 0 ){      //***BUG: should be '> 0' & not '>= 0' ***//
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ){
    ...
    case adventurer:
      adventurerEffect(drawntreasure, currentPlayer, nextPlayer, state, temphand);
      return 0;
    ...
    case minion:
      minionEffect(currentPlayer, choice1, choice2, handPos, state);
      discardCard(handPos, currentPlayer, state, 0);
      return 0;
    ...
    case cutpurse:
      cutPurseEffect(currentPlayer, handPos, state);
      return 0;
